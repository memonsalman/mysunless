	...
	<div class="preloader">
									...
									<div class="tab-content tabcontent-border">
										...
										<div class="tab-pane active" id="UpcomingRenewalsTab" role="tabpanel">
											<div class="col-lg-12 col-md-12 p-2">
												<button id="UpcomingRenewalsTableSubmit" class="btn btn-info">Send</button>
												<div class="progress_box mt-2" id="progress_box_renewals" style="display: none;">
													<div class="progress">
														<div id="sendloader_renewals" class="progress-bar bg-primary" role="progressbar" style="width: 0%; height: 6px;" aria-valuenow="2" aria-valuemin="0" aria-valuemax="1000"></div>
													</div>
													<span id="CountSend_renewals" style="font-size: 20px;"></span>
												</div>
												<div class="d-flex align-items-center">
													<select class="custom-select col-md-3 ml-auto" id="UpcomingRenewalsDays">
												...
												<div class="table-responsive">
													<table id="UpcomingRenewalsTable" class="table table-bordered table-striped dataTable no-footer" style="width: 100%;">
														<thead>
															<tr>
																<th><input type="checkbox" name="" id="UpcomingRenewalsTable-select-all"></th>
																<th>User Info</th>
																<th>Contact Info</th>
																<th>Package Name</th>
																<th>Start Date</th>
		...
		<script type="text/javascript">
			$(document).ready(function(){
                ...
                var UpcomingRenewalsTable= UpcomingRenewalsDataTable(15);
                $('#UpcomingRenewalsDays').change(function(){
                	var UpcomingRenewalsDays = (this).value ;
                	UpcomingRenewalsTable=UpcomingRenewalsDataTable(UpcomingRenewalsDays);
                }
                );
                function UpcomingRenewalsDataTable(UpcomingRenewalsDays){
                	var UpcomingRenewalsTable= $('#UpcomingRenewalsTable').DataTable({
                		"order": [[ 5, 'asc' ]],
                		"processing" : true,
                		"destroy": true,
                		"responsive": true,
                		...
                		"columns" : [
                		{
                			...
                			{
                				return '<input type="checkbox" value="'+data.UserID+'" class="UpcomingRenewalsTableCheckbox">';   
                			}
                		},
                		{
                			...
                			"render": function(data, type, row) {
                				if(data.userimg != '' && data.userimg != null){
                					return '<div class=""> <img src="<?php echo base_url; ?>/assets/userimage/'+data.userimg+'" class="img-circle" style="height: 50px; width: 50px;" /> <br><span style="text-transform: capitalize;">'+data.username+'</span> </div>';
                				}
                				else
                				{
                					return '<div class=""> <img src="<?php echo base_url; ?>/assets/images/'+'noimage.png'+'" class="img-circle" style="height: 50px; width: 50px;" /> <br><span style="text-transform: capitalize;">'+data.username+'</span> </div>';
                				}
                			}
                		},
                		{
                			...
                			"render": function(data, type, row) {
                				...
                				{
                					return '<span class=""><b>Email: </b>'+data.email;
                				}
                				else
                				{
                					return '<span class=""><b>Email: </b>'+data.email+'</span><br><span class=""><b>Phone: </b>'+data.phonenumber+'</span>';
                				}
                			}
                		},
                		{
                		...
                		},
                		{
                			...
                			"render":function (data,type,row)
                			{
                				return moment(data.paytime).format('YYYY-MM-DD');
                			}
                		},
                		{
                			...
                			"render":function (data,type,row)
                			{
                				return moment(data.package_ed).format('YYYY-MM-DD');   
                			}
                		},
                		{
                			"data": {UserID:"UserID"},
                ...
                $('#UpcomingRenewalsTable tbody').on( 'click', '.sendSubscription', function (e) {
                	e.preventDefault();
                ...
                $('#UpcomingRenewalsTable-select-all').on('click', function(e){
                	var rows = UpcomingRenewalsTable.rows({ 'search': 'applied' }).nodes();
                	$('.UpcomingRenewalsTableCheckbox', rows).prop('checked', this.checked);
                });

                $('#UpcomingRenewalsTable tbody').on('change', '.UpcomingRenewalsTableCheckbox', function(){
                	if(!this.checked){
                		var el = $('#UpcomingRenewalsTable-select-all').get(0);
                ...
                var ret=0;
                var totalret = 0; 
                $('#UpcomingRenewalsTableSubmit').on('click', function(e){
                	e.preventDefault();
                	var data_idArray = [];
                	countret = 0;
                	...
                	UpcomingRenewalsTable.$('input[type="checkbox"]').each(function(){
                	...
                	$("#progress_box_renewals").show();
                	
                	$("#CountSend_renewals").text("0/"+totalret);
                	...
                	for(i=0; i<data_idArray.length;i++){
                		$.ajax({
                			...
                			success: function(data){
                				ret +=1;
                				$("#CountSend_renewals").text(ret+"/"+totalret);
                				temp = ret*100/totalret;
                				loader(temp); 
                				
                				if(ret==totalret){
                					$("#sendloader_renewals").css('width','0%');
                					$('.UpcomingRenewalsTableCheckbox').prop('checked',false);
                					swal("",'Mail Successfully sent!!', "");
                					$("#progress_box_renewals").hide();
                					$("#CountSend_renewals").text("0/0");
                					ret=0;
                					data_idArray.length=0;
                ...
                function loader(ret){
                	$("#sendloader_renewals").css('width',ret+"%");
                }
            });
        </script>
        <script type="text/javascript">
