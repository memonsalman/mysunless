...
class Campaigns{
    ...
    public function __construct($myid = "new"){
        ...
        if ($myid == "new") {
            ...
            try {
                $query = $db->prepare("SELECT * FROM Campaigns_Temp WHERE id=:myid");
    ...
    public function commit(){
        ...
        if ($this->id == "new") {
            try {
                $query = $db->prepare("INSERT INTO `Campaigns_Temp` (`TempName`) VALUES ('New')");
                $query->execute();
        ...
        try {
            $query = $db->prepare("UPDATE Campaigns_Temp SET 
                `TempName`=:Campaigns,
        ...
        $isactive=1;
        $id=$_SESSION['UserID'];
        $LoginQuery = $db->prepare("SELECT * FROM `Campaigns_Temp` WHERE isactive=:isactive");
