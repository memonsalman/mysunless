...
private function Regstration_Validation_for_passwoed_update()
{
    ...
    $checkpwd=$db2->prepare("SELECT password FROM `Check_Password` WHERE user_id=:id");
    $checkpwd->bindparam(':id',$id,PDO::PARAM_STR);
    $checkpwd->execute();
    ...
    else
    {
	    ...
	    {
	    	$ans=$checkpwd->fetchAll();
	    	$b=$ans[0]['password']; 

	    	if($b==md5($_REQUEST['confirm_password']))
	    	{
	    		$Result['check_exists_password'] = "This New password has been used previously,Please try new Password.";
	    	}		
	    }
    }
...
public function update_password()
{
    ...
    if(isset($_SESSION['UserID']))
    {
    	...
    	$cpassword=md5($_REQUEST['current_password']);
        ...
        if($stmt)
        {
           $checkall=$db5->prepare("SELECT password from Check_Password where user_id=:userid");
           $checkall->bindparam(':userid',$id,PDO::PARAM_INT);
           $checkall->execute();
        	if($checkall->rowCount()>0)
	    	{
	    		$addpwd=$db5->prepare("UPDATE Check_Password set password=:cpassword where user_id=:uid"); 	    		      	
	        	$addpwd->bindparam(':uid',$id,PDO::PARAM_INT);        	
	        	$addpwd->bindparam(':cpassword',$cpassword,PDO::PARAM_STR);
	        	$addpwd->execute();
	    	}
	    	else
        	{
        		$addpwd=$db5->prepare("INSERT INTO Check_Password(user_id,password) VALUES(:uid,:cpassword)");        	
	        	$addpwd->bindparam(':uid',$id,PDO::PARAM_INT);        	
	        	$addpwd->bindparam(':cpassword',$cpassword,PDO::PARAM_STR);
	        	$addpwd->execute();  
        	}
    ...
    if(isset($_SESSION['UserID']))
        {       $db4=new db();
            ...
            $stmt=$db4->prepare("UPDATE `users` SET 
                ...
                lastname=:lastname, email=:email, phonenumber=:phonenumber, companyname=:companyname, companytype=:companytype, companywebsite=:companywebsite, companywebsite=:companywebsite, primaryaddress=:primaryaddress, secondaryaddress=:secondaryaddress, zipcode=:zipcode, city=:city, state=:state, country=:country,NotificationStatus=:notification where id=:id");
