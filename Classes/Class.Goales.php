	...
	public function __construct($myid = "new"){
		...
		if ($myid == "new") {
			...
			try {
                ...
                $query = $db->prepare("SELECT * FROM goles WHERE id=:myid");
			...
			while ($info = $query->fetch(PDO::FETCH_ASSOC)) {
				...
				$this->MonthlyGoal = $info["MonthlyGoal"];

				$this->YearlyGoal = $info["YearlyGoal"];
	 ...
	 public function commit(){
        ...
        if ($this->id == "new") {
            ...
            try {
                ...
                $query = $db->prepare("INSERT INTO `goles` (`MonthlyGoal`) VALUES ('New')");
		...
		try {
            ...
            $query = $db->prepare("UPDATE goles SET 
			 ...
			 `isactive`=:isactive  WHERE id=:myid");
			  ...
			  $query->bindValue(':MonthlyGoal', $this->MonthlyGoal, PDO::PARAM_STR);

			  $query->bindValue(':YearlyGoal', $this->YearlyGoal, PDO::PARAM_STR);
